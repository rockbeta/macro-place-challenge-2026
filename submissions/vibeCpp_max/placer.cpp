...
constexpr int ALL_MACRO_SA_MAX_ITERS_DEFAULT = 1600000;
...
constexpr int GLOBAL_MOVE_FINE_GRID_DEFAULT = 40;
...
int default_thread_count() {
    ...
    return std::max(1, std::min(8, static_cast<int>(hc)));
}
...
bool has_hard_overlaps_float32(
    const BenchmarkData& b,
    const std::vector<Point>& pos,
    float gap = 0.0f
) {
    for (int i = 0; i < b.n_hard; ++i) {
        for (int j = i + 1; j < b.n_hard; ++j) {
            float dx = std::abs(static_cast<float>(pos[i].x) - static_cast<float>(pos[j].x));
            float dy = std::abs(static_cast<float>(pos[i].y) - static_cast<float>(pos[j].y));
            float sep_x = (static_cast<float>(b.macros[i].w) + static_cast<float>(b.macros[j].w)) * 0.5f + gap;
            float sep_y = (static_cast<float>(b.macros[i].h) + static_cast<float>(b.macros[j].h)) * 0.5f + gap;
...
bool has_output_hard_overlaps(const BenchmarkData& b, const std::vector<Point>& pos) {
    return has_hard_overlaps(b, pos, 0.0) || has_hard_overlaps_float32(b, pos, 0.0f);
}
...
void legalize_to_boundary(
    ...
    for (int idx : order) {
        ...
        for (const auto& item : bounds) {
            Point p;
            if (try_place_on_boundary(item.second, cx, cy, w, h, b.cw, b.ch, placed, gap, p)) {
                pos[idx] = p;
                placed.emplace_back(p.x - w * 0.5, p.x + w * 0.5, p.y - h * 0.5, p.y + h * 0.5);
                done = true;
                break;
            }
        }
...
void spiral_legalize(
    ...
    auto overlaps = [&](int idx, double x, double y) {
        for (int j : placed_idx) {
            ...
            if (std::abs(x - legal[j].x) < sep_x && std::abs(y - legal[j].y) < sep_y) {
                return true;
            }
        }
        return false;
    ...
    for (int idx : order) {
        ...
        double step = std::max(std::min(b.macros[idx].w, b.macros[idx].h) * 0.25, 0.05);
        Point best{x0, y0};
...
void push_legalize(
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            pos[i].x = clamp(pos[i].x + force[i].x, b.macros[i].w * 0.5 + gap, b.cw - b.macros[i].w * 0.5 - gap);
            pos[i].y = clamp(pos[i].y + force[i].y, b.macros[i].h * 0.5 + gap, b.ch - b.macros[i].h * 0.5 - gap);
        }
    }
}

void robust_legalize_hard(const BenchmarkData& b, std::vector<Point>& pos, double gap = GAP) {
...
void legalize_large_then_small(
    const BenchmarkData& b,
    std::vector<Point>& pos,
    ...
    for (int attempt = 0; attempt < 3 && has_hard_overlaps(b, pos, 0.0); ++attempt) {
        std::vector<bool> fixed(b.n_hard, false);
        for (int i = 0; i < b.n_hard; ++i) {
            fixed[i] = b.macros[i].fixed;
        }
...
Point pin_xy(const BenchmarkData& b, const std::vector<Point>& pos, int pin_idx) {
...
double net_hpwl(const BenchmarkData& b, const std::vector<Point>& pos, int net_i) {
...
public:
    ...
    IncrementalDensityCost(const BenchmarkData& bench, const std::vector<Point>& start_pos)
        ...
        top_count = std::max(1, static_cast<int>(std::floor(b.grid_rows * b.grid_cols * DENSITY_TOP_FRAC)));
    ...
    std::pair<double, Token> begin_single_update(int idx, double nx, double ny) {
        ...
        if (old_rng.valid || new_rng.valid) {
            tok.r0 = std::min(old_rng.valid ? old_rng.r0 : new_rng.r0, new_rng.valid ? new_rng.r0 : old_rng.r0);
            tok.r1 = std::max(old_rng.valid ? old_rng.r1 : new_rng.r1, new_rng.valid ? new_rng.r1 : old_rng.r1);
            tok.c0 = std::min(old_rng.valid ? old_rng.c0 : new_rng.c0, new_rng.valid ? new_rng.c0 : old_rng.c0);
            tok.c1 = std::max(old_rng.valid ? old_rng.c1 : new_rng.c1, new_rng.valid ? new_rng.c1 : old_rng.c1);
    ...
    std::pair<double, Token> begin_two_update(
        int a,
        double ax,
        double ay,
        int c,
    ...
    void accept_two(const Token& tok, int a, double ax, double ay, int c, double cx, double cy) {
        current_cost = tok.new_cost;
        pos[a] = {ax, ay};
        pos[c] = {cx, cy};
    }
...
private:
    ...
    Range macro_cell_range(int idx, double cx, double cy) const {
        ...
        double ly = cy - h * 0.5;
        double uy = cy + h * 0.5;
        if (w <= 0.0 || h <= 0.0 || ux <= 0.0 || uy <= 0.0 || lx >= b.cw || ly >= b.ch) {
            return {};
        }
        Range out;
        out.valid = true;
        out.c0 = std::max(0, std::min(b.grid_cols - 1, static_cast<int>(std::floor(lx / grid_w))));
        out.c1 = std::max(0, std::min(b.grid_cols - 1, static_cast<int>(std::floor(ux / grid_w))));
        out.r0 = std::max(0, std::min(b.grid_rows - 1, static_cast<int>(std::floor(ly / grid_h))));
        out.r1 = std::max(0, std::min(b.grid_rows - 1, static_cast<int>(std::floor(uy / grid_h))));
        return out;
    }
    ...
    void add_macro(int idx, double cx, double cy, double sign) {
        ...
        int c0 = std::max(0, std::min(b.grid_cols - 1, static_cast<int>(std::floor(lx / grid_w))));
        int c1 = std::max(0, std::min(b.grid_cols - 1, static_cast<int>(std::floor(ux / grid_w))));
        int r0 = std::max(0, std::min(b.grid_rows - 1, static_cast<int>(std::floor(ly / grid_h))));
        int r1 = std::max(0, std::min(b.grid_rows - 1, static_cast<int>(std::floor(uy / grid_h))));
        for (int r = r0; r <= r1; ++r) {
            double y0 = r * grid_h;
            double y1 = y0 + grid_h;
...
struct RouteKey {
    ...
    bool operator==(const RouteKey& other) const {
        return valid == other.valid && source == other.source && cells == other.cells;
    }
};
...
public:
    struct Token {
        double old_cost = 0.0;
        double new_cost = 0.0;
        std::vector<std::tuple<int, NetContrib, NetContrib, RouteKey, RouteKey>> nets;
    ...
    IncrementalCongestionCost(const BenchmarkData& bench, std::vector<Point>& positions)
        ...
        abu_top_count = std::max(1, static_cast<int>(std::floor(2.0 * grid_size * CONGESTION_TOP_FRAC)));
    ...
    std::pair<double, Token> begin_single_update(int moved_macro, const std::vector<int>& affected_nets) {
        ...
        for (int ni : nets) {
            ...
            NetContrib next;
            if (new_key.valid) {
                next = net_contrib_from_gcells(ni, std::get<1>(key_data), std::get<2>(key_data));
            }
    ...
    std::pair<double, Token> begin_multi_update(
        std::vector<int> moved_macros,
        ...
        moved_macros.erase(std::unique(moved_macros.begin(), moved_macros.end()), moved_macros.end());
    ...
    std::vector<int> hot_congestion_nets() const {
        ...
        auto hotter = [&](int a, int c) {
            return combined[a] > combined[c];
        };
        if (take < static_cast<int>(order.size())) {
            std::nth_element(order.begin(), order.begin() + take, order.end(), hotter);
        } else {
...
private:
    ...
    std::pair<int, int> grid_cell(double x, double y) const {
        int row = std::max(0, std::min(b.grid_rows - 1, static_cast<int>(std::floor(y / grid_h))));
        int col = std::max(0, std::min(b.grid_cols - 1, static_cast<int>(std::floor(x / grid_w))));
        return {row, col};
    }
    ...
    void route_two(NetContrib& contrib, std::pair<int, int> source, const std::vector<std::pair<int, int>>& cells, double weight) const {
        if (cells.size() < 2) {
            return;
        }
    ...
    void route_l(NetContrib& contrib, std::vector<std::pair<int, int>> cells, double weight) const {
        std::sort(cells.begin(), cells.end(), [](auto a, auto c) {
            return std::make_pair(a.second, a.first) < std::make_pair(c.second, c.first);
        });
        int y1 = cells[0].first, x1 = cells[0].second;
        int y2 = cells[1].first, x2 = cells[1].second;
        int y3 = cells[2].first, x3 = cells[2].second;
        for (int col = x1; col < x2; ++col) add_h_route(contrib, y1, col, weight);
        for (int col = x2; col < x3; ++col) add_h_route(contrib, y2, col, weight);
        for (int row = std::min(y1, y2); row < std::max(y1, y2); ++row) add_v_route(contrib, row, x2, weight);
        for (int row = std::min(y2, y3); row < std::max(y2, y3); ++row) add_v_route(contrib, row, x3, weight);
    }

    void route_t(NetContrib& contrib, std::vector<std::pair<int, int>> cells, double weight) const {
        ...
        for (int col = xmin; col < xmax; ++col) add_h_route(contrib, y2, col, weight);
        for (int row = std::min(y1, y2); row < std::max(y1, y2); ++row) add_v_route(contrib, row, x1, weight);
        for (int row = std::min(y2, y3); row < std::max(y2, y3); ++row) add_v_route(contrib, row, x3, weight);
    }

    void route_three(NetContrib& contrib, std::vector<std::pair<int, int>> cells, double weight) const {
        std::sort(cells.begin(), cells.end(), [](auto a, auto c) {
            return std::make_pair(a.second, a.first) < std::make_pair(c.second, c.first);
        });
        int y1 = cells[0].first, x1 = cells[0].second;
        ...
        if (x1 < x2 && x2 < x3 && std::min(y1, y3) < y2 && std::max(y1, y3) > y2) {
            ...
            for (int col = x1; col < x2; ++col) add_h_route(contrib, y1, col, weight);
            for (int row = y1; row < std::max(y2, y3); ++row) add_v_route(contrib, row, x2, weight);
        } else if (y2 == y3) {
            for (int col = x1; col < x2; ++col) add_h_route(contrib, y1, col, weight);
            for (int col = x2; col < x3; ++col) add_h_route(contrib, y2, col, weight);
            for (int row = std::min(y2, y1); row < std::max(y2, y1); ++row) add_v_route(contrib, row, x2, weight);
        } else {
            route_t(contrib, cells, weight);
        }
    }

    std::pair<std::pair<int, int>, std::vector<std::pair<int, int>>> get_net_gcells(int net_i) const {
        ...
        return {source, std::vector<std::pair<int, int>>(unique_cells.begin(), unique_cells.end())};
    }

    std::tuple<RouteKey, std::pair<int, int>, std::vector<std::pair<int, int>>> net_route_key_data(int net_i) const {
    ...
    NetContrib net_contrib_from_gcells(int net_i, std::pair<int, int> source, const std::vector<std::pair<int, int>>& cells) const {
        NetContrib contrib;
        double weight = (0 <= net_i && net_i < static_cast<int>(b.net_weights.size())) ? b.net_weights[net_i] : 1.0;
        if (cells.size() == 2) {
    ...
    std::pair<NetContrib, RouteKey> net_contrib(int net_i) const {
        ...
        return {net_contrib_from_gcells(net_i, std::get<1>(data), std::get<2>(data)), key};
    }
    ...
    MacroContrib macro_contrib(int macro_i) const {
        ...
        for (int row = bl.first; row <= ur.first; ++row) {
            for (int col = bl.second; col <= ur.second; ++col) {
                ...
                if (ur.first != bl.first && (row == bl.first || row == ur.first) && std::abs(y_dist - grid_h) > 1e-5) {
                    partial_vertical = true;
                }
                if (ur.second != bl.second && (col == bl.second || col == ur.second) && std::abs(x_dist - grid_w) > 1e-5) {
                    partial_horizontal = true;
                }
                out.v.push_back({row, col, x_dist * VROUTING_ALLOC * inv_grid_v_routes});
                out.h.push_back({row, col, y_dist * HROUTING_ALLOC * inv_grid_h_routes});
            }
        }
        if (partial_vertical) {
            int row = ur.first;
    ...
    void accum_net_delta(const NetContrib& contrib, double sign, std::vector<double>& vd, std::vector<double>& hd) const {
        for (const auto& s : contrib.v) {
            for (int c = s.c_lo; c < s.c_hi; ++c) {
...
std::vector<Point> sa_all_macro_incremental(
    const BenchmarkData& b,
    std::vector<Point> pos,
    const std::vector<bool>& fixed_override,
    int seed,
    double congestion_weight,
    double density_weight,
    double hard_move_prob,
    double hard_sigma_start,
    double soft_sigma_start,
    int max_iters,
    Bounds bounds,
    bool log_progress,
    ...
    for (int i = 0; i < b.n_hard; ++i) {
        ...
        bool allowed = allowed_mask == nullptr ||
                       (i < static_cast<int>(allowed_mask->size()) && (*allowed_mask)[i]);
        if (!fixed && allowed) movable_hard.push_back(i);
    }
    for (int i = b.n_hard; i < b.n_total; ++i) {
        bool fixed = fixed_override.empty() ? b.macros[i].fixed : fixed_override[i];
        bool allowed = allowed_mask == nullptr ||
                       (i < static_cast<int>(allowed_mask->size()) && (*allowed_mask)[i]);
        if (!fixed && allowed) movable_soft.push_back(i);
    }
    ...
    double cur_cost = std::accumulate(cur_hpwl.begin(), cur_hpwl.end(), 0.0) +
                      (congestion_weight * cong_eval.current() + density_weight * den_eval.current()) * wl_norm;
    ...
    const std::vector<double>& move_scales =
        (move_scales_override != nullptr && !move_scales_override->empty())
            ...
            : default_move_scales;
    bool gate_default_rescue_scales = move_scales_override == nullptr || move_scales_override->empty();
    ...
    auto log_proxy = [&](const std::string& label) {
        ...
        std::cerr << "[vibeCpp] " << label << " proxy=" << proxy
                  << " WL=" << wl << " density=" << den_eval.current()
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        ...
        int idx;
        double sigma;
        if (!movable_hard.empty() && (movable_soft.empty() || uni(rng) < hard_move_prob)) {
        ...
        for (double move_scale : move_scales) {
            ...
            if (idx < b.n_hard) {
                ...
                for (int j = 0; j < b.n_hard; ++j) {
                    ...
                    if (std::abs(nx - pos[j].x) < sep_x && std::abs(ny - pos[j].y) < sep_y) {
                        bad = true;
                        break;
                    }
                }
        ...
        if (iter % 1000 == 0 && recent_costs.size() == 1000) {
            double prev = 0.0;
            double curr = 0.0;
            for (int i = 0; i < 500; ++i) prev += recent_costs[i];
            for (int i = 500; i < 1000; ++i) curr += recent_costs[i];
            prev /= 500.0;
            curr /= 500.0;
            ...
            if (std::isfinite(grad_abs)) {
                if (grad_abs >= grad_ref) {
                    ...
                    frac = std::log(grad_ref / grad_abs) / std::log(grad_ref / grad_stop_threshold);
...
std::vector<Point> parallel_sa_all_macro(
    ...
    if (threads <= 1 || max_iters < 2000) {
        return sa_all_macro_incremental(
            ...
            hard_move_prob, hard_sigma_start, soft_sigma_start, max_iters,
            bounds, log_progress, allowed_mask, move_scales_override);
    }
    ...
    for (int worker = 0; worker < threads; ++worker) {
        ...
        futures.push_back(std::async(
            std::launch::async,
            [&, worker_iters, worker_seed]() {
                return sa_all_macro_incremental(
                    b, start_pos, fixed_override, worker_seed,
                    congestion_weight, density_weight, hard_move_prob,
                    hard_sigma_start, soft_sigma_start, worker_iters,
                    bounds, false, allowed_mask, move_scales_override);
            }));
    }
    ...
    if (log_progress) {
        std::cerr << "[vibeCpp] " << label << " parallel SA final proxy="
                  << best_proxy.proxy << " WL=" << best_proxy.wirelength
...
bool macro_center_inside_canvas(const BenchmarkData& b, int idx, Point p) {
    double hw = b.macros[idx].w * 0.5;
    double hh = b.macros[idx].h * 0.5;
    return p.x >= hw + 1e-6 && p.x <= b.cw - hw - 1e-6 &&
           p.y >= hh + 1e-6 && p.y <= b.ch - hh - 1e-6;
}

bool hard_pair_overlaps_at(
    const BenchmarkData& b,
    int a,
    Point pa,
    int c,
    ...
    float fsep_x = (static_cast<float>(b.macros[a].w) + static_cast<float>(b.macros[c].w)) * 0.5f;
    float fsep_y = (static_cast<float>(b.macros[a].h) + static_cast<float>(b.macros[c].h)) * 0.5f;
    return dx < fsep_x && dy < fsep_y;
}

bool swap_has_hard_overlap(const BenchmarkData& b, const std::vector<Point>& pos, int a, int c) {
    Point pa = pos[c];
    Point pc = pos[a];
    if (!macro_center_inside_canvas(b, a, pa) || !macro_center_inside_canvas(b, c, pc)) {
        return true;
    }
...
std::vector<int> affected_nets_for_pair(const BenchmarkData& b, int a, int c) {
    ...
    affected.insert(affected.end(), b.macro_to_nets[c].begin(), b.macro_to_nets[c].end());
    std::sort(affected.begin(), affected.end());
    affected.erase(std::unique(affected.begin(), affected.end()), affected.end());
    return affected;
}

std::vector<Point> swap_sa_polish(
    const BenchmarkData& b,
    std::vector<Point> pos,
    int seed,
    int max_iters
    ...
    int window_count = std::max(1, static_cast<int>(by_area.size()) - group_size + 1);
    ...
    for (int iter = 1; iter <= max_iters; ++iter) {
        ...
        for (int u = 0; u < group_size; ++u) {
            for (int v = u + 1; v < group_size; ++v) {
                ...
                auto den_pair = den_eval.begin_two_update(a, pos[a].x, pos[a].y, c, pos[c].x, pos[c].y);
                auto den_token = den_pair.second;
        ...
        if (best_a >= 0 && best_proxy.proxy < current.proxy - SWAP_SA_ACCEPT_TOL) {
            ...
            auto den_pair = den_eval.begin_two_update(
        ...
        if (iter % SWAP_SA_LOG_INTERVAL == 0 || iter == max_iters) {
            double success_rate = static_cast<double>(accepted) / static_cast<double>(iter);
            double valid_rate = static_cast<double>(valid_rounds) / static_cast<double>(iter);
            ...
            if (iter >= SWAP_SA_LOG_INTERVAL && success_rate < SWAP_SA_MIN_SUCCESS_RATE) {
                std::cerr << "[vibeCpp] swap SA early stop success_rate=" << success_rate
                          << " threshold=" << SWAP_SA_MIN_SUCCESS_RATE
                          << " iter=" << iter << "\n";
                break;
            }
        }
...
std::vector<bool> hot_congestion_macro_mask(
    const BenchmarkData& b,
    std::vector<Point>& pos,
    int& hot_net_count,
    int& hard_count,
...
std::vector<Point> hot_congestion_sa_polish(
    const BenchmarkData& b,
    ...
    std::vector<bool> hot_macros =
        hot_congestion_macro_mask(b, start, hot_net_count, hot_hard_count, hot_soft_count);
    int hot_macro_count = hot_hard_count + hot_soft_count;
    if (hot_macro_count == 0) {
        std::cerr << "[vibeCpp] hot congestion SA skipped hot_nets=" << hot_net_count
                  << " hot_macros=0\n";
        return positions;
    }
    ...
    std::vector<Point> candidate = parallel_sa_all_macro(
        b, positions, {}, seed,
        HOT_CONGESTION_SA_CONG_WEIGHT, HOT_CONGESTION_SA_DENSITY_WEIGHT,
        0.55, 0.015, 0.045, max_iters, Bounds{}, true,
        "hot_congestion_sa", &hot_macros, &hot_move_scales);
    ...
    std::cerr << "[vibeCpp] hot congestion SA "
              ...
              << " proxy " << current.proxy << " -> " << after.proxy
              << " congestion " << current.congestion << " -> " << after.congestion
...
std::vector<Point> place(const BenchmarkData& b, int seed) {
    ...
    std::cerr << "[vibeCpp] initial proxy=" << start.proxy << " WL=" << start.wirelength
              << " density=" << start.density << " congestion=" << start.congestion << "\n";
    ...
    int short_iters = env_int("VIBECPP_SA_SHORT_ITERS", ALL_MACRO_SA_SHORT_ITERS_DEFAULT);
    int full_iters = env_int("VIBECPP_SA_FULL_ITERS", ALL_MACRO_SA_MAX_ITERS_DEFAULT);
    ...
    int global_move_fine_grid = env_int("VIBECPP_GLOBAL_MOVE_FINE_GRID", GLOBAL_MOVE_FINE_GRID_DEFAULT);
    int global_move_fine_rows = env_int("VIBECPP_GLOBAL_MOVE_FINE_GRID_ROWS", global_move_fine_grid);
    int global_move_fine_cols = env_int("VIBECPP_GLOBAL_MOVE_FINE_GRID_COLS", global_move_fine_grid);
    int final_fix_iters = env_int("VIBECPP_FINAL_FIX_SA_ITERS", ALL_MACRO_SA_MAX_ITERS_DEFAULT);

    positions = parallel_sa_all_macro(
        b, positions, {}, seed + 505,
        ALL_MACRO_SA_CONG_WEIGHT, ALL_MACRO_SA_DENSITY_WEIGHT,
        0.35, 0.03, 0.08, short_iters, Bounds{}, true,
        "short_all_macro");
    clamp_positions(b, positions, original);
    legalize_large_then_small(b, positions, large_mask, GAP);

    positions = parallel_sa_all_macro(
        b, positions, {}, seed + 1505,
        ALL_MACRO_SA_CONG_WEIGHT, ALL_MACRO_SA_DENSITY_WEIGHT,
        0.35, 0.03, 0.08, full_iters, Bounds{}, true,
        "full_all_macro");
    clamp_positions(b, positions, original);

    ProxyComponents current = compute_proxy(b, positions);
    if (cong_iters > 0 && current.congestion >= CONGESTION_POLISH_MIN_CONGESTION) {
        for (int pass = 0; pass < 2; ++pass) {
            double cong_weight = pass == 0 ? 0.9 : 1.2;
            std::vector<Point> candidate = parallel_sa_all_macro(
                b, positions, {}, seed + 2505 + pass * 101,
                cong_weight, PROXY_DENSITY_WEIGHT,
                0.45, 0.015, 0.045, cong_iters, Bounds{}, false,
                "congestion_polish");
            clamp_positions(b, candidate, original);
            if (has_output_hard_overlaps(b, candidate)) {
                legalize_large_then_small(b, candidate, large_mask, GAP);
            ...
            ProxyComponents after = compute_proxy(b, candidate);
            bool accept = after.proxy <= current.proxy + CONGESTION_POLISH_ACCEPT_TOL &&
                          after.congestion <= current.congestion + EPS;
    ...
    positions = hot_congestion_sa_polish(
    ...
    positions = global_grid_move_polish(
        b, positions, seed + 4305, global_move_fine_rows, global_move_fine_cols);
    clamp_positions(b, positions, original);

    positions = parallel_sa_all_macro(
        b, positions, {}, seed + 4505,
        ALL_MACRO_SA_CONG_WEIGHT, ALL_MACRO_SA_DENSITY_WEIGHT,
        0.35, 0.03, 0.08, final_fix_iters, Bounds{}, true,
        "full_all_macro");
    clamp_positions(b, positions, original);

    for (int attempt = 0; attempt < 3 && has_output_hard_overlaps(b, positions); ++attempt) {
        legalize_large_then_small(b, positions, large_mask, GAP);
        clamp_positions(b, positions, original);
    }
    clamp_positions(b, positions, original);
    ProxyComponents final = compute_proxy(b, positions);
    std::cerr << "[vibeCpp] final proxy=" << final.proxy << " WL=" << final.wirelength
              << " density=" << final.density << " congestion=" << final.congestion
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--input" && i + 1 < argc) {
            ...
            std::cerr << "usage: vibe_placer --input benchmark.txt --output placement.txt [--seed N]\n";
            return 2;
        }
    }
