...
def _score_seed_candidate_worker(
    benchmark: Benchmark,
    seed: int,
    idx: int,
    out_path: Path,
    ...
    try:
        ...
        sizes_np = benchmark.macro_sizes.detach().cpu().numpy().astype(np.float64)
        pos_np = candidate.detach().cpu().numpy().astype(np.float64)
        score = _proxy_components_for_acceptance(
            benchmark,
            pos_np,
            sizes_np,
...
class VibeCppPlacer:
    def __init__(self, seed: int = 7):
        ...
        self._second_seed_sweep_count = max(
            0,
            _env_int("VIBECPP_SECOND_SEED_SWEEP_COUNT", self._seed_sweep_count),
        )
        self._second_knob_perturb_rel_std = max(
            0.0,
            _env_float("VIBECPP_SECOND_KNOB_PERTURB_REL_STD", 0.30),
        )
        self._third_seed_sweep_count = max(
            0,
            _env_int("VIBECPP_THIRD_SEED_SWEEP_COUNT", self._seed_sweep_count),
        )
        self._third_knob_perturb_rel_std = max(
            0.0,
            _env_float("VIBECPP_THIRD_KNOB_PERTURB_REL_STD", 0.40),
        )
        self._final_topk = max(1, _env_int("VIBECPP_FINAL_TOPK", 4))
        total_seed_sweep_count = (
            self._seed_sweep_count
            + self._second_seed_sweep_count
            + self._third_seed_sweep_count
        )
        default_score_workers = min(
            total_seed_sweep_count,
            max(1, os.cpu_count() or 1),
        )
        self._score_workers = max(
            1,
            _env_int("VIBECPP_SCORE_WORKERS", default_score_workers),
        )
    ...
    def _sweep_jobs(self) -> List[dict]:
        jobs = []
        global_idx = 1
        seed_offset = 0
        batches = [
            (1, self._seed_sweep_count, KNOB_PERTURB_REL_STD),
            (2, self._second_seed_sweep_count, self._second_knob_perturb_rel_std),
            (3, self._third_seed_sweep_count, self._third_knob_perturb_rel_std),
        ]
        active_batch_count = sum(1 for _round_id, count, _std in batches if count > 0)
        for round_id, count, rel_std in batches:
            if count <= 0:
                continue
            for round_idx in range(1, count + 1):
                seed = self.seed + seed_offset * self._seed_sweep_stride
                jobs.append(
                    {
                        "idx": global_idx,
                        "seed": seed,
                        ...
                        "round_idx": round_idx,
                        "round_count": count,
                        "active_rounds": active_batch_count,
                        "rel_std": rel_std,
                        "baseline": round_id == 1 and round_idx == 1,
                    }
                )
                global_idx += 1
                seed_offset += 1
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        sweep_jobs = self._sweep_jobs()
        with tempfile.TemporaryDirectory(prefix="vibecpp_") as tmpdir_s:
            tmpdir = Path(tmpdir_s)
            print(
                "[vibeCpp] parallel seed sweep enabled | "
                f"batches={max((job['round'] for job in sweep_jobs), default=0)} "
                f"count={len(sweep_jobs)} "
                f"seeds={','.join(str(job['seed']) for job in sweep_jobs)}",
                flush=True,
            )
            ...
            for job in sweep_jobs:
                ...
                tag = (
                    "baseline"
                    if job["baseline"]
                    else f"perturbed sigma={rel_std:.2f}"
                )
                print(
                    f"[vibeCpp] prepared worker {idx}/{len(sweep_jobs)} | "
                    f"round={job['round']}/{job['active_rounds']} "
                    f"round_idx={job['round_idx']}/{job['round_count']} "
            ...
            for round_id in sorted({int(job["round"]) for job in sweep_jobs}):
                batch_indices = [
                    i for i, job in enumerate(sweep_jobs)
                    if int(job["round"]) == round_id
                ]
                if not batch_indices:
                    continue
                rel_std = float(sweep_jobs[batch_indices[0]]["rel_std"])
                print(
                    "[vibeCpp] launching seed sweep batch | "
                    f"round={round_id}/{sweep_jobs[batch_indices[0]]['active_rounds']} "
                    f"count={len(batch_indices)} perturb_sigma={rel_std:.2f}",
                    flush=True,
                )
                processes = []
                ...
                for i in batch_indices:
                    ...
                    print(
                        f"\n{'═' * 72}\n"
                        f"[vibeCpp] ── worker {job['idx']}/{len(sweep_jobs)} "
                        f"round={job['round']}/{job['active_rounds']} "
                        f"seed={job['seed']} log ──\n"
                        f"{'═' * 72}",
                        flush=True,
            ...
            score_workers = min(self._score_workers, len(worker_args))
            print(
                "[vibeCpp] scoring seed sweep candidates | "
                f"workers={score_workers}",
                flush=True,
            )
            score_results = []
            if score_workers <= 1:
                for job, args in zip(sweep_jobs, worker_args):
                    score_results.append(
                        _score_seed_candidate_worker(
                            benchmark,
                            int(job["seed"]),
                            int(job["idx"]),
                            args[9],   # out_path
            ...
            jobs_by_idx = {int(job["idx"]): job for job in sweep_jobs}
            for result in sorted(score_results, key=lambda item: item["idx"]):
                idx = int(result["idx"])
...
ADAM_GAMMA_SCALE_MULT = 1.0
EXACT_DENSITY_GAMMA_SCALE_MULT = 1.0
ADAM_PERTURB_SCALE_MULT = 1.0
...
#   "mult"  → clamp into (0.25, 2.5)  — for schedule shape multipliers
...
_KNOB_DEFAULTS = {
    ...
    "step3_cong_weight":                 (STEP3_CONG_WEIGHT, "float"),
    ...
    "step6_cong_weight":                 (STEP6_CONG_WEIGHT, "float"),
    ...
    "exact_density_cong_weight":         (EXACT_DENSITY_CONG_WEIGHT, "float"),
    ...
    "adam_gamma_scale_mult":             (ADAM_GAMMA_SCALE_MULT, "mult"),
    "exact_density_gamma_scale_mult":    (EXACT_DENSITY_GAMMA_SCALE_MULT, "mult"),
    "adam_perturb_scale_mult":           (ADAM_PERTURB_SCALE_MULT, "mult"),
}
...
def _clamp_knob(value: float, kind: str) -> float:
    ...
    if kind == "mult":
        return float(min(2.5, max(0.25, value)))
...
def _adam_phase(
    ...
    macro_blockage_weight: float = CONG_MACRO_BLOCKAGE_WEIGHT,
    gamma_scale_mult: float = ADAM_GAMMA_SCALE_MULT,
    perturb_scale_mult: float = ADAM_PERTURB_SCALE_MULT,
) -> np.ndarray:
    print(
    ...
    gamma_start = canvas * 0.05 * float(gamma_scale_mult)
    gamma_end = canvas * 0.005 * float(gamma_scale_mult)
    ...
    for it in range(iters):
        ...
        if perturbation:
            ...
            if (
                ...
                perturb_scale = (
                    _perturb_scale(perturb_progress)
                    * float(perturb_scale_mult)
                )
...
def _adam_exact_density_phase(
    ...
    perturbation: bool = True,
    macro_blockage_weight: float = CONG_MACRO_BLOCKAGE_WEIGHT,
    gamma_scale_mult: float = EXACT_DENSITY_GAMMA_SCALE_MULT,
    perturb_scale_mult: float = ADAM_PERTURB_SCALE_MULT,
) -> np.ndarray:
    print(
        f"[adam] {label} starting | iters={iters} "
    ...
    half_h = sizes_t[:, 1] / 2
    opt = torch.optim.Adam([pos_var], lr=canvas * lr_frac)
    gamma_start = canvas * 0.02 * float(gamma_scale_mult)
    gamma_end = canvas * 0.002 * float(gamma_scale_mult)
...
class VibePlacer:
    ...
    def _step3_adam_large(
        ...
        return _adam_phase(
            ...
            cong_weight=knobs["step3_cong_weight"],
            net_weights_np=official_net_weights_np,
            label="step3_adam_large",
            perturbation=True,
            macro_blockage_weight=early_blockage,
            gamma_scale_mult=knobs["adam_gamma_scale_mult"],
            perturb_scale_mult=knobs["adam_perturb_scale_mult"],
        )
    ...
    def _step6_adam_small_and_exact(
        ...
        positions = _adam_phase(
            ...
            cong_weight=knobs["step6_cong_weight"],
            net_weights_np=official_net_weights_np,
        ...
        positions = _adam_exact_density_phase(
            ...
            cong_weight=knobs["exact_density_cong_weight"],
            net_weights_np=official_net_weights_np,
            label="step6_adam_exact_density",
            gamma_scale_mult=knobs["exact_density_gamma_scale_mult"],
            perturb_scale_mult=knobs["adam_perturb_scale_mult"],
        )
        return positions
